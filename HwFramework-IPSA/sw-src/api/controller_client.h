...
namespace api {
    ...
    public:
        ...
        RC load_neuron_primitive_context(const NeuronPrimitiveContext& ctx) {
            rp4::LoadNeuronPrimitiveContextReq request;
            auto *ctx_msg = request.mutable_context();
            ctx_msg->set_contextid(ctx.context_id);
            ctx_msg->set_numinputs(ctx.num_inputs);
            ...
            for (auto weight : ctx.weights) {
                ctx_msg->add_weights(weight);
            }
            for (auto bias : ctx.biases) {
                ctx_msg->add_biases(bias);
            }

            rp4::LoadNeuronPrimitiveContextResp response;
            ClientContext context;
            Status status = stub_->loadNeuronPrimitiveContext(&context, request, &response);
            if (!status.ok()) {
                std::cerr << "loadNeuronPrimitiveContext RPC failed for ctx "
                          << ctx.context_id << ": "
                          << status.error_code() << " "
                          << status.error_message() << std::endl;
                return 0;
            }
            if (response.res() != rp4::Res::OK) {
                std::cerr << "loadNeuronPrimitiveContext rejected ctx "
                          << ctx.context_id << std::endl;
                return 0;
            }
            return 1;
        }

        RC load_sigmoid_table(int value_bitwidth,
                              uint32_t input_multiplier,
                              const std::vector<std::pair<int32_t, uint32_t>>& points) {
            rp4::LoadSigmoidTableReq request;
            request.set_value_bitwidth(value_bitwidth);
            request.set_inputmultiplier(input_multiplier);
            for (const auto &point : points) {
                auto entry = request.add_points();
                entry->set_input(point.first);
                entry->set_value(point.second);
            }

            rp4::LoadSigmoidTableResp response;
            ClientContext context;
            Status status = stub_->loadSigmoidTable(&context, request, &response);
            if (!status.ok()) {
                std::cerr << "loadSigmoidTable RPC failed: "
                          << status.error_code() << " "
                          << status.error_message() << std::endl;
                return 0;
            }
            if (response.res() != rp4::Res::OK) {
                std::cerr << "loadSigmoidTable rejected entries (missing contiguous inputs?)"
                          << std::endl;
                return 0;
            }
            return 1;
        ...
        RC clear_sigmoid_table() {
            rp4::ClearSigmoidTableReq request;
            rp4::ClearSigmoidTableResp response;
            ClientContext context;
            Status status = stub_->clearSigmoidTable(&context, request, &response);
            if (!status.ok()) {
                return 0;
            }
            return response.res() == rp4::Res::OK ? 1 : 0;
        }

        RC load_exp_table(uint32_t input_multiplier,
                          uint32_t value_scale,
                          const std::vector<std::pair<int32_t, uint32_t>>& points) {
            rp4::LoadExpTableReq request;
            request.set_inputmultiplier(input_multiplier);
            request.set_valuescale(value_scale);
            for (const auto &point : points) {
                auto entry = request.add_points();
                entry->set_input(point.first);
                entry->set_value(point.second);
            ...
            rp4::LoadExpTableResp response;
            ClientContext context;
            Status status = stub_->loadExpTable(&context, request, &response);
            if (!status.ok()) {
                return 0;
            }
            return response.res() == rp4::Res::OK ? 1 : 0;
        ...
        RC clear_exp_table() {
            rp4::ClearExpTableReq request;
            rp4::ClearExpTableResp response;
            ClientContext context;
            Status status = stub_->clearExpTable(&context, request, &response);
            if (!status.ok()) {
                return 0;
            }
            return response.res() == rp4::Res::OK ? 1 : 0;
        ...
        RC clear_neuron_primitive_contexts() {
            rp4::ClearNeuronPrimitiveContextsReq request;
            rp4::ClearNeuronPrimitiveContextsResp response;
            ClientContext context;
            Status status = stub_->clearNeuronPrimitiveContexts(&context, request, &response);
            if (!status.ok()) {
                return 0;
            }
            return response.res() == rp4::Res::OK ? 1 : 0;
        ...
        static rp4::GateEntryType get_gate_entry_type (GateEntryType get) {
            switch (get) {
                ...
                default : return rp4::GateEntryType::TABLE;
            }
        }
    ...
    private:
        ...
        static rp4::OpCode get_op_code(const OpCode & op) {
            switch (op) {
                ...
                case OpCode::MUL : return rp4::OpCode::MUL;
                case OpCode::DIV : return rp4::OpCode::DIV;
                case OpCode::NEURON_PRIMITIVE : return rp4::OpCode::NEURON_PRIMITIVE;
                case OpCode::SIGMOID_LOOKUP : return rp4::OpCode::SIGMOID_LOOKUP;
                case OpCode::SUM_BLOCK : return rp4::OpCode::SUM_BLOCK;
                default: return rp4::OpCode::ADD;
            }
        }

        static rp4::ActivationFunction get_activation(ActivationFunction activation) {
            switch (activation) {
                case ActivationFunction::RELU: return rp4::ActivationFunction::ACT_RELU;
                case ActivationFunction::SIGMOID: return rp4::ActivationFunction::ACT_SIGMOID;
                case ActivationFunction::NONE:
                default: return rp4::ActivationFunction::ACT_NONE;
            }
        }
