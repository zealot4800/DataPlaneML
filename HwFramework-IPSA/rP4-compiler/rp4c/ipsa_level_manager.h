...
void IpsaLevelManager::load(const Rp4Ast* ast) {
    ...
    auto get_select = [&](const Rp4StateDef* state_def) -> std::vector<IpsaHeaderField> {
        ...
        for (auto& target : targets) {
            if (auto field = header_manager->lookup(target.member_name, target.field_name)) {
    ...
    auto build_entries = [&](IpsaLevel& level, int state, const std::vector<Rp4TransitionEntry>& entries) {
        for (auto& entry : entries) {
            if (entry.accept_drop == 2) {
                ...
                const IpsaHeader* m = nullptr;
                int header_id = 0;
                int header_length = 0;
                auto& target = next_state->extract.target;
                if (!target.instance_name.empty()) {
                    m = header_manager->get_header(&target);
                }
                if (m != nullptr) {
                    header_id = m->header_id;
                    header_length = m->offset;
                }
