    ...
    for (auto & parser_json : j.at("parsers")) {
        ...
        for (auto & parse_state_json : parser_json.at("parse_states")) {
            ...
            for (auto & parse_op_json : parse_state_json.at("parser_ops")) {
                auto & param_json = parse_op_json.at("parameters").at(0).at("value");
                if (!param_json.is_string()) {
                    continue;
                }
                ParserOperation parser_op;
                parser_op.parameter_name = param_json.get<std::string>();
                parse_state.parser_ops.push_back(parser_op);
            }
            for (auto & transition_json : parse_state_json.at("transitions")) {
                Transition transition;
                std::string transition_type = transition_json.value("type", std::string(""));
                transition.value = safe_convert(transition_json.at("value"));
                if (transition.value.empty() && transition_type == "default") {
